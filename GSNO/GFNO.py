...
def test_model():
    ...
    # Initialize tensors and move them to the specified device
    u_in = torch.randn(batch_size, n_points, T_in, num_features, device=device)
    P = torch.randn(batch_size, num_parameters, device=device)
    pos = torch.randn(batch_size, n_points, 2, device=device)

    # Print the shapes of the inputs
    print("\nInput shapes:")
    print(f"x: {u_in.shape}")
