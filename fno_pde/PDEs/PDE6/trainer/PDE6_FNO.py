...
if (enable_ig_loss == False):
    Mode = Operator + f"_Data_zoned_33_{n_zones}_{train_sample}"
    ss = 1
# Data + IG
if (enable_ig_loss == True):
    Mode = Operator + f"_Data_IG_zoned_33_{n_zones}_{train_sample}" 
    ss = 1 + 1
...
# %%
filename = f'datasets/main_dataset_zones_3_{n_zones}.pt'
...
for ep in outer_loop:
    ...
    for batch_data in train_loader:
        ...
        num_samples_x = 20  # Number of random samples along the second axis (x)
        num_samples_t = 20  # Number of random samples along the third axis (t)
