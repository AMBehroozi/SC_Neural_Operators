...
from models.Multipole_Graph_Neural_Operator import MGNO1DTime
Operator = 'MGNO'
...
eval_dataset = torch.load( PATH +  'datasets/evaldataset.pt')
train_loader = DataLoader(train_dataset, batch_size=batch_size, shuffle=True)
eval_loader = DataLoader(eval_dataset, batch_size=batch_size)

# %%
width, depth = 128, 8
model = MGNO1DTime(nx, T_in, T_out, state_size, parameter_size, width, depth).to(device)
...
optimizer = optim.Adam([
                {'params': model.parameters(), 'lr': learning_rate, 'weight_decay': 1e-3},
...
for ep in outer_loop:
    ...
    for batch_data in train_loader:
        ...
        U_in = batch_u_in
        U_pred = model(U_in, batch_parameters)  #u, x, t, par
