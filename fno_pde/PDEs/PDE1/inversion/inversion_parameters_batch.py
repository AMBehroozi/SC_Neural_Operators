...
mu_target = parameter_target[:, 5:6].detach()
...
# SC-FNO  
    # Batch Average Results:
    # Average Absolute Difference: 9.0402e-03
    # Average Relative Error (%): 18.842
    # R-squared: 0.9920

# FNO
    # Batch Average Results:
    # Average Absolute Difference: 3.0110e-02
    # Average Relative Error (%): 50.465
    # R-squared: 0.7985


# %%
loss_list1 = torch.load('FNO.pt')

loss_list2 = torch.load('SC_FNO.pt')

# Moving tensors to CPU and converting to a list of scalars
loss_list_cpu1 = [loss.item() for loss in loss_list1]
loss_list_cpu2 = [loss.item() for loss in loss_list2]

# Plotting both sets of loss values
plt.figure(figsize=(10, 6))
plt.plot(loss_list_cpu1, marker='o', linestyle='-', color='b', label='FNO')
plt.plot(loss_list_cpu2, marker='o', linestyle='-', color='r', label='SC-FNO')
plt.title('Comparison of Loss Values Over Iterations')
